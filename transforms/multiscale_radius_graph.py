...
class MultiscaleRadiusGraph(object):
    ...
    def __init__(self, ratios, radii, max_neighbours=512, sample_n=None, loop=False, flow='source_to_target',
                 cache_file=None):
    ...
    def __call__(self, data):
        ...
        for i, r in enumerate(self.ratios):
            ...
            if r == 1:
                ...
                pool_neigh = torch.from_numpy(vh.nearest(pos_vh, face_vh, idx_vh, labels_vh)).round().long().clamp(0,
                                                                                                                   pool_idx.size(
                                                                                                                       0) - 1).view(
                    -1)
                ...
                edge_index = torch.cat(
                    (edge_index, torch.stack((original_idx[pool_idx][pool_neigh][sample_idx], original_idx), dim=0)),
                    dim=1)
                # Add corresponding mask entries for pooling edges
                edge_mask = torch.cat((edge_mask, (torch.ones(sample_idx.size(0)) * (0b1 << (i * 2))).long()))

            # Sample nodes
            sample_idx, original_idx, pos, batch = sample_idx[pool_idx], original_idx[pool_idx], pos[pool_idx], batch[
                pool_idx] if batch is not None else None
    ...
    def __repr__(self):
        return '{}(radii={}, ratios={}, sample_n={})'.format(self.__class__.__name__, self.radii, self.ratios,
                                                             self.sample_n)
