...
class SURFMNetLoss(nn.Module):
    ...
    def forward(self, C1, C2, feat_1, feat_2, evecs_1, evecs_2, evals_1, evals_2, device):
        ...
        laplacian_penalty = criterion(torch.einsum('abc,ac->abc', C1, evals_1),
                                      torch.einsum('ab,abc->abc', evals_2, C1))
        laplacian_penalty += criterion(torch.einsum('abc,ac->abc', C2, evals_2),
                                       torch.einsum('ab,abc->abc', evals_1, C2))
        ...
        source1, target1 = torch.einsum('abcde,abcef->abcdf', C1_expand, M_1), torch.einsum('abcef,abcfd->abced', M_2,
                                                                                            C1_expand)
        source2, target2 = torch.einsum('abcde,abcef->abcdf', C2_expand, M_2), torch.einsum('abcef,abcfd->abced', M_1,
                                                                                            C2_expand)
