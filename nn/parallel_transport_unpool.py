...
class ParallelTransportUnpool(Module):
    ...
    def forward(self, x, data):
        ...
        unpool_nodes, unpool_edges, unpool_connection = data.unpool_nodes[-1], data.unpool_edges[-1], \
                                                        data.unpool_connection[-1]
        if len(data.unpool_nodes) > 1:
            data.unpool_nodes, data.unpool_edges, data.unpool_connection = data.unpool_nodes[:-1], data.unpool_edges[
        ...
        if x.size(1) > 1:
            x[:, 1, :, 0], x[:, 1, :, 1] = complex_product(x[:, 1, :, 0], x[:, 1, :, 1], connection[:, None, 0],
                                                           -connection[:, None, 1])
        return x
